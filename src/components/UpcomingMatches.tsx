...
export function UpcomingMatches() {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        setLoading(true);
        // Fetch upcoming matches from both APIs
        const [sportmonksMatches, cricapiMatches] = await Promise.all([
          sportmonks.getUpcomingMatches(),
          cricapi.getUpcomingMatches(),
        ]);
  ...
  if (matches.length === 0) {
    return <div className="text-center text-muted-foreground">No upcoming matches available.</div>;
  }
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {matches.map((match) => (
